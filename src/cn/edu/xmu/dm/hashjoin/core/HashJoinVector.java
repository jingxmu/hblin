...
import cn.edu.xmu.dm.hashjoin.calculatesimilarity.CalVectorSim;
...
import cn.edu.xmu.dm.hashjoin.minhashing.MinHashing_Vector;
...
public class HashJoinVector {
	...
	public static void main(String[] args) throws IOException,
		...
		MinHashing_Vector.run(conf);//generate source vector		
		String sourcePath = conf.get("DATA_DIR")
				+ conf.get("FEATURE_VECTOR_PATH") + "/part-m-00000";
		DistributedCache.addCacheFile(new Path(sourcePath).toUri(), conf);
		
		CalVectorSim.run(conf);
